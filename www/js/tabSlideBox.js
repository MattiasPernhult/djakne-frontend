...
['$timeout', '$compile', '$interval', '$ionicSlideBoxDelegate',
'$ionicScrollDelegate', '$ionicGesture', function($timeout, $compile, $interval,
$ionicSlideBoxDelegate, $ionicScrollDelegate, $ionicGesture) {
  return {
    require: '^ionSlideBox',
    restrict: 'A',
    link: function(scope, element, attrs, parent) {
      ...
      var setIndicatorPosition = function(currentSlideIndex, targetSlideIndex,
        ...
        if (currentSlideIndex === 0 && targetSlideIndex ===
          ionicSlideBoxDelegate.slidesCount() - 1 && slideDirection === 'right' ||
          targetSlideIndex === 0 && currentSlideIndex ===
          ionicSlideBoxDelegate.slidesCount() - 1 && slideDirection === 'left') {
          return;
        }
      ...
      scope.addTabContent = function($content) {
        ...
        $timeout(function() {
          slideTabs =
          angular.element(tabsBar[0].querySelector('ul').querySelectorAll('.slider-slide-tab'));
          slideToCurrentPosition();
          setTabBarWidth();
        });
      ...
      scope.onSlideMove = function() {
        ...
        var currentSlideLeftOffset =
        currentSlide.css('-webkit-transform').replace(/[^0-9\-.,]/g, '').split(',')[0];
